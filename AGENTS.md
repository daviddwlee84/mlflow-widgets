...
Triggers on push to `main` (or manual `workflow_dispatch`). Builds MkDocs and deploys to GitHub Pages using `actions/deploy-pages`. The built site includes LLM-friendly outputs: `/llms.txt` (index), `/llms-full.txt` (all pages concatenated), and per-page `.md` files generated by `mkdocs-llmstxt`. A "Copy to LLM" button is also injected on every page via `mkdocs-copy-to-llm`.
